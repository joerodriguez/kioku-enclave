...
| Runtime ownership | Private inactive Phase-1 lease-lifecycle owner and separate authoritative launcher; advisory release cannot enter the existing authority importer. The owner-only drain binds a complete prefix to the exact pinned legacy transaction, and the private recovery/replay/parity worker returns only a non-settling commitment. | Prove one archive/one owner, maintenance-window and zero-serving-replica preconditions, durable comparison/restart ownership, a distinct exact Phase-2 authority acquisition, comparison handoff behavior, and no second Store/runtime authority. |
