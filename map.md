...
| `src/archive_v3_wal_idempotency.rs` / `src/archive_v3_wal_owner.rs` | **Inactive ADR-0022 logical mutation and single-archive owner:** fixed portable operation domains, opaque stable IDs/fingerprints, bounded retained replay results, and sealed per-domain ledgers feed a private actor whose dedicated blocking lane owns the recovered SQLite copy and reversible exact-one WAL drain. Encrypted control persists archive+kind+operation-scoped owner/publication/attempt/artifact facts, transition-validates the deterministic segment/descriptor/root plan, and reconciles an exact pending candidate successor before settlement. Fresh witnessed current staging supplies permanent indexed replay without replacing another operation's terminal Control row. Only test domain/provider implementations exist; ordinary Store remains legacy, and no production provider, runtime/startup/route/config, acknowledgement, list/delete, cloud, or serving wiring exists. |
