...
| `.github/workflows/` | CI, CodeQL/dependency checks, image build/scan, provenance/SBOM attestations, and the signed schema-v4 Phase-0 media-bucket release-manifest subject |
| `Dockerfile` | Digest-pinned builder/model definition for the static `x86_64-unknown-linux-musl` image; Phase-0 requires the baked media bucket to equal the index bucket, with remaining rebuild limits documented in `SECURITY.md` |
