...
| `src/archive_v3_shadow_runtime.rs` | **Conditionally active sealed ADR-0022 runtime composer:** the signed Genesis profile binds fixed archive-GCS, registry-KMS, and named-Firestore providers to the one durable archive commitment for Genesis and WAL serving, and startup separately installs an exact deletion-only bundle using the same baked coordinates plus Control-derived deletion/page keys. Wrong archive bindings refuse before provider construction; off-profile images install no deletion lane and retain the honest pending response. |
