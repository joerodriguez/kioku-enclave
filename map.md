...
canary scope plus a separate runtime-precondition row whose consumption is atomic with
the first owner reservation. Its private three-root operator/image/deployment-observer
verifier fixes Phase 1 to no authoritative mutations, legacy-only acknowledgements, and
...
| `src/archive_v3_maintenance_import.rs` | **Inactive ADR-0022 maintenance-window import and Phase-1 bootstrap:** a sealed-runtime-owned, single-archive offline coordinator durably fences and pins one exact legacy Store generation, authenticates the existing Active+Legacy witness/root/registry, uploads exact-readback checkpoint objects under a distinct domain-separated zero-WAL-only binding and canonical R1, reconciles ShadowWal only by exact witness reread, and performs full independent SQLite parity with fresh source/witness validation. A type-separated advisory importer stops at that verified ShadowWal point, exact-releases only the importer lease, revalidates the legacy generation, scrubs DB/WAL/SHM, drops its owned Store guards, and returns a sealed Store/user/archive/import target with no authority conversion. The target remains fail-closed until the separate inactive release ledger freezes the owner; it can then exact-read/authenticate and delete only its one recorded provider-marker generation, reconciling response loss by exact-name absence. A distinct terminal transition freshly authenticates the same witness/Control row, installs one exact-user capture selector, and reopens both local legacy gates together; its owner can detach only an opaque cancellation-safe prefix bound to the exact read-only legacy snapshot established while the Store actor is serialized. The private advisory child exact-recovers R1, replays that prefix, runs full independent parity, and returns only a non-settling commitment after repeated source/release/witness validation. Maintenance performs its post-lifecycle-lock release check before closing either gate, so stale work cannot reblock the user. The distinct offline R2 path still reaches WalAuthoritative when selected from its own valid predecessor, but direct continuation through it after advisory release is fenced pending a separately reviewed Phase-2 acquisition transition. Encrypted control owns bounded restart stages, full-tuple lease renewal/reacquire CAS, reacquire-only partial-attempt supersession, up to 16 retained attempts, and 32,898 exact artifacts per attempt. It has no main/startup/route/worker/config/serving wiring, archive-v3 provider list/delete, legacy-source deletion, durable capture settlement, policy switch, cloud action, or deployment authority; only the existing bounded legacy-intent prefix scan drains pre-marker writes. |
