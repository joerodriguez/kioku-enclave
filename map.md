...
| `src/archive_v3_deletion.rs` | Inactive ADR-0022 deletion driver: freshly witness-reauthenticated worker/operation/fence orchestration over a sealed, bounded canonical metadata inventory; only inventory-minted per-entry capabilities reach the concrete GCS adapter, which validates full execution binding and exact membership before I/O. It removes exact content generations and derived permanent claims, reconciles uncertain mutations only by exact absence checks, and binds physical-completion witness evidence to the exact inventory plus a fresh provider drain. It has no route, Store, runtime/provider/credential construction, or authority wiring. Activation remains compile-time blocked: the separate reachability visitor is non-authorizing, the inventory API is module-private, and no production create-ahead union/page plan or non-test full-reachability seal exists. |
| `src/archive_v3_lifecycle.rs` / `src/archive_v3_lifecycle_page_store.rs` | Inactive ADR-0022 lifecycle authority/codec plus the separately encrypted external inventory-page producer. Before page admission, control durably commits the exact settled artifact/witness snapshot and closes later reconciliation. The page seam derives per-page keys and domain-separated retry-stable nonces from a control-store-proven DEK, binds the exact full-hash name/reference/fence in AEAD, persists at most one unresolved bounded page so its exact split survives restart, reconciles only by exact authenticated readback, and permits exact all-generation/soft-delete cleanup only after durable-control physical completion freezes and provider-drains every prior create. It exposes no listing, walker, runtime/provider construction, or production authority. |
