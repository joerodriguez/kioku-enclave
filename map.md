...
| `src/archive_v3_wal_idempotency.rs` / `src/cp/media/wal.rs` / `src/cp/media/wal/capture_event.rs` / `src/cp/media/wal/media_dek.rs` / `src/cp/model_usage/wal.rs` / `src/cp/query/wal.rs` / `src/cp/query/wal/selected_screenshot_attempt.rs` / `src/cp/query/wal/finalization_queue.rs` / `src/cp/finalizer/wal.rs` / `src/cp/media_worker/wal.rs` / `src/cp/media_worker/wal/attempt.rs` / `src/cp/media_worker/wal/result.rs` / `src/cp/email_worker/wal.rs` / `src/cp/push/wal.rs` / `src/cp/webhook_worker/wal.rs` / `src/cp/reviewer/wal.rs` / `src/cp/summarizer/wal.rs` / `src/cp/summarizer/wal/visual_evidence.rs` / `src/archive_v3_wal_owner.rs` / `src/archive_v3_wal_owner/publisher.rs` | **Inactive ADR-0022 logical mutation, single-archive owner, and publisher/checkpoint worker:** fixed portable operation domains, opaque stable IDs/fingerprints, bounded retained replay results, and sealed per-domain ledgers feed a private actor whose dedicated blocking lane owns the recovered SQLite copy and reversible exact-one WAL drain. Fifteen production A seals cover the reviewed deterministic local settlements. The media-DEK B child accepts only a future KMS-supplied wrapped/plaintext pair, immediately derives a keyed account/wrapper binding, retains no plaintext key, and first-writer-wins installs or exact-adopts only the wrapped value with a one-row commitment-only replay ledger; its KMS producer remains absent. The screen Vertex B child authenticates one exact reserved topology with a fixed two-minute lease window, binds its exact predecessor and post-usage-stable attempt identity, derives a stable Vertex event ID from account/work/attempt identity, and atomically records the started billing event, deterministic monthly coverage, complete binding, and typed replay receipt. A renewed lease/counter topology produces a new identity. The production-facing screen-result v2 reauthenticates that permanent binding and exact stable attempt topology before any result write or replay; historical v1 identity/encoding remains test-only. A second B child consumes a caller-fixed selected-screenshot upload attempt, commits the exact screenshot identity and eligible predecessor, derives the account-bound object key, and atomically reserves pending episode count/bytes before provider I/O. The production-facing selected-screenshot A v2 has a distinct result domain, reconstructs and consumes that exact permanent B binding before local settlement and on replay, and retains historical unbound v1 only for tests; exact A consumption avoids double-counting, while absent/inexact consumption stays reserved until a future C settlement. Canonical encryption/upload, provider receipt authentication, rejection release, screen person evidence, and every audio/person/identity/voice result remain unsupported. Provider calls, deletion, delivery, media reads, automatic IDs/clocks, retry, Store, launching, and acknowledgement stay outside the logical codecs. Encrypted control persists archive+kind+operation-scoped publication facts, transition-validates the deterministic WAL plan, and reconciles an exact pending candidate successor before settlement. The private publisher consumes only the maintenance handoff, manages adequate-lifetime/same-fence/higher-fence witness leases, atomically consumes terminal comparison rows before later owner-binding transitions, and performs mandatory checkpoint source retirement on owned blocking workers plus exact deterministic reserve/create/readback recovery before a new mutation crosses fixed caps. Candidate/send source maintenance reads and authenticates exact provider state without renewing/reacquiring; reload recomputes the full chunk/manifest/root topology, lost-success reconciliation precedes the old-head gate, and checkpoint settlement atomically consumes the prior witnessed logical publication. The runtime exposes only exact-name immutable create/get. Fresh witnessed current staging supplies permanent indexed replay without replacing another operation's terminal Control row. No launcher exists; ordinary Store remains legacy, and no runtime/startup/route/config, acknowledgement, list/delete, cloud, or serving wiring exists. |
