...
and routed Store reads/submits serve only settled owner state. The checked-in default image
profile remains exact `off`; an active image is possible only through the reviewed signed
archive-v3 release profile and fails closed when its baked coordinates disagree with durable
state. This active logical-WAL path is distinct from the old advisory/extent migration stack.
...
| `src/archive_v3_firestore_shadow.rs` | **Conditionally active Firestore witness composition:** the signed runtime constructs the exact named-database adapter, dedicated attestation bearer, and fixed transport without I/O for Genesis and WAL publication. Off-profile images construct none of it. |
...
| `src/archive_v3_shadow_runtime.rs` | **Conditionally active sealed ADR-0022 single-archive WAL runtime:** under an exact signed active image profile, startup and the Genesis trigger synchronously derive the fixed archive-GCS, exact registry-KMS-version, and named-Firestore providers without I/O, then require an exact image-commitment match against the opaque durable encrypted-control archive binding. The non-cloneable provider bundle can be consumed only into Genesis creation or a durable WAL serving handoff; archive IDs/providers stay private with no general getters, arbitrary callbacks, or deletion authority. Checked-in default configuration remains exact `off`, and off-config startup refuses if any durable selected user exists. The old maintenance/advisory and extent-shadow consumers remain inactive or deleted. |
