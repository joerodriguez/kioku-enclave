...
| `src/archive_v3_wal_idempotency.rs` | **Inactive ADR-0022 WAL logical-mutation gate:** fixed portable operation domains, nonzero opaque IDs, domain/version-separated request fingerprints, bounded canonical replay results, and a sealed contract requiring each future domain to own a distinct hard-capped exact-indexed ledger. Only a bounded test exemplar exists; there is no universal or production receipt table. Store retains legacy snapshot persistence in every production constructor; a test-only WAL policy rejects missing-user genesis before KMS/temp/upload and opens existing checkpointed SQLite immutably without sidecars, while generic mutations, dirty saves/evictions, envelope rewrites, and schema migration fail closed. No publisher, capture consumption, root/witness transition, route, worker, startup, provider, cloud, or deployment wiring exists. |
