...
| Path | What it is |
|---|---|
| [config/](config/map.md) | Checked-in, non-secret, fail-closed attested-image configuration; the archive-witness probe and construction-only archive-v3 runtime profiles start exact `off`/empty and have no repository-variable or dispatch override |
...
| `src/archive_v3_deletion.rs` | Inactive ADR-0022 deletion driver: freshly witness-reauthenticated worker/operation/fence orchestration over the exact authenticated lifecycle-page inventory; only inventory-minted per-entry capabilities reach the concrete GCS adapter, which validates full execution binding and exact membership before I/O. It removes exact content generations and derived permanent claims, reconciles uncertain mutations only by exact absence checks, and binds physical-completion witness evidence to the lifecycle seal's exact commitment plus a fresh provider drain. The obsolete independent builder/commitment is gone. It has no route, Store, runtime/provider/credential construction, invocation, or authority wiring. |
| `src/archive_v3_lifecycle.rs` / `src/archive_v3_lifecycle_page_store.rs` / `src/archive_v3_inventory_coordinator.rs` | Inactive ADR-0022 lifecycle authority/codec, separately encrypted exact-name inventory-page producer, and authenticated inventory coordinator. Final KILP-v2 pages contain only canonical key/role/ciphertext-hash facts; create attempt/ordinal/state/length remain solely in the frozen create-ahead snapshot. Exact Tombstoned worker authority is authenticated before control freeze and again before the graph's first GET. The coordinator visits current/predecessor graphs, unions all settled create-ahead facts by exact object ID, deterministically pages them, revalidates witness/control freshness before page I/O and sealing, and supplies the control CAS a one-shot private proof binding the exact snapshot, plan, references, and authenticated readbacks; no raw pages-to-seal surface remains. It loads the sealed chain after restart with the lifecycle commitment unchanged. Per-page keys/nonces bind the exact full-hash reference/fence; at most one unresolved page retains exact retry bytes. Cleanup still requires durable-control physical completion and all-generation/soft-delete drain. There is no list API, pre-witness absence path, startup/Store/runtime/provider construction, deletion invocation, cloud implementation, or production authority. |
