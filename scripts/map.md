...
| `release.sh` | Verifies the selected voice-quality classification, repository-bound billing mode, and GitHub-signed schema-v5 Phase-0 dual-media-bucket manifest; creates a signed tag, verifies build evidence, publishes an immutable release, and optionally requests an operator roll. |
