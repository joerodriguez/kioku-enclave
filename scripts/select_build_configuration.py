...
# Read-only compatibility for the live v0.9.18 predecessor while v0.9.19 rolls
# out. This is the exact schema-12/operator keyspace and must never inherit new
# current-profile keys implicitly.
FROZEN_V0_9_18_TAG = "v0.9.18"
FROZEN_V0_9_18_SHARED_KEYS = (
    "PROJECT_ID",
    "REGION",
    "AR_REPOSITORY",
    "IMAGE_NAME",
)
FROZEN_V0_9_18_PROFILE_KEYS = (
    "ENCLAVE_KMS_PROJECT",
    "ENCLAVE_KMS_LOCATION",
    "ENCLAVE_KMS_KEY_RING",
    "ENCLAVE_KMS_KEY",
...
FROZEN_V0_9_18_OPTIONAL_PROFILE_GROUPS = (
    (
        "APPLE_TEAM_ID",
        "APPLE_KEY_ID",
        "APPLE_IOS_CLIENT_ID",
...
FROZEN_V0_9_18_OPERATOR_CONFIG_KEYS = frozenset(
    (*FROZEN_V0_9_18_SHARED_KEYS, "LOCAL_GCP_IMPERSONATE_SERVICE_ACCOUNT")
    + tuple(
        f"{profile}_{key}"
        for profile in ("PRODUCTION", "EVALUATION")
        for key in (
            ...
            *(
                key
                for group in FROZEN_V0_9_18_OPTIONAL_PROFILE_GROUPS
                for key in group
            ),
        )
    )
...
def parse_frozen_v0_9_18_operator_configuration(data: bytes) -> dict[str, str]:
    """Parse only the operator-config keyspace accepted by the v0.9.18 tag."""
    try:
        text = data.decode("utf-8")
    except UnicodeDecodeError as error:
        raise SystemExit("operator configuration must be UTF-8 text") from error
    ...
    for line_number, line in enumerate(text.splitlines(), start=1):
        ...
        if name not in FROZEN_V0_9_18_OPERATOR_CONFIG_KEYS:
            raise SystemExit(
                f"unknown operator configuration name at line {line_number}"
            )
        if any(ord(character) < 32 or ord(character) == 127 for character in value):
...
def validate(configuration: dict[str, str], profile: str, source_ref: str) -> None:
    ...
    if (
        not re.fullmatch(r"[1-9][0-9]{0,15}", configuration["QUOTA_VERTEX_OUTPUT_TOKENS_PER_DAY"])
        or configuration["QUOTA_VERTEX_OUTPUT_TOKENS_PER_DAY"]
        != REVIEWED_VERTEX_OUTPUT_TOKENS_PER_DAY
    ):
        raise SystemExit(
            "QUOTA_VERTEX_OUTPUT_TOKENS_PER_DAY must match the reviewed daily limit"
        )
...
def validate_frozen_v0_9_18_configuration(
    configuration: dict[str, str], profile: str, source_ref: str
) -> None:
    """Apply the selector validation captured by the immutable v0.9.18 tag."""
    if source_ref != FROZEN_V0_9_18_TAG:
        raise SystemExit(
            "frozen v0.9.18 build configuration requires the exact v0.9.18 tag"
        )
    reject_control_characters(configuration)
    require_pattern(configuration, "PROJECT_ID", PROJECT_PATTERN)
    require_pattern(configuration, "REGION", r"[a-z]+-[a-z0-9]+[0-9]")
    ...
    if profile == "production" and not configuration.get(
        "VERTEX_RECONCILIATION_MODEL"
    ):
        raise SystemExit(
            "production builds require an explicit reconciliation model and compiled producer contract"
    ...
    if not re.fullmatch(RELEASE_TAG_PATTERN, source_ref):
        raise SystemExit("release source_ref is not a canonical version tag")


def selected_frozen_v0_9_18_configuration(
    profile: str,
    environment: dict[str, str],
    *,
    source_ref: str,
) -> dict[str, str]:
    """Select the exact live v0.9.18 schema-12 configuration."""
    if source_ref != FROZEN_V0_9_18_TAG:
        raise SystemExit(
            "frozen v0.9.18 build configuration requires the exact v0.9.18 tag"
        )
    ...
    configuration = {
        name: require_value(environment, name)
        for name in FROZEN_V0_9_18_SHARED_KEYS
    }
    for name in FROZEN_V0_9_18_PROFILE_KEYS:
        configuration[name] = require_value(environment, f"{prefix}_{name}")
    for group in FROZEN_V0_9_18_OPTIONAL_PROFILE_GROUPS:
        values = {name: environment.get(f"{prefix}_{name}", "") for name in group}
        if any(values.values()) and not all(values.values()):
    ...
    validate_frozen_v0_9_18_configuration(configuration, profile, source_ref)
    return configuration
    ...
    configuration.update(
        {
            ...
            "DRAIN_TIMEOUT_SECONDS": "105",
            "ENCLAVE_TLS": "1",
            "QUOTA_VERTEX_OUTPUT_TOKENS_PER_DAY": REVIEWED_VERTEX_OUTPUT_TOKENS_PER_DAY,
        }
    )
