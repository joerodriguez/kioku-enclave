#!/usr/bin/env python3
"""Sign the ADR-0022 Phase-1 advisory-canary three-root authorization.

Solo-operator tool per docs/adr/0022-solo-operator-activation.md: the one
operator produces the exact 242-byte operator statement, 82-byte image
attestation, and 298-byte runtime admission that
`verify_pinned_advisory_canary_authorization` authenticates against the three
pinned roots, and signs each under its domain with the matching operator-held
private key.

- Writes six raw evidence files (payload + signature per root) into
  `--output-dir`, exactly the files `--run-archive-v3-phase1-canary` consumes.
...
Every field is an exact-length hex string; zero values the verifier would
reject are refused here, so an unusable authorization cannot be produced. The
runtime admission hard-codes the Phase-1 facts the verifier requires: the
empty authoritative mutation set, the `PHASE1_ADVISORY` marker, and
legacy-only acknowledgement.
"""
...
OPERATOR_STATEMENT_DOMAIN = b"kioku/archive-v3/advisory-canary/operator-statement/v1\x00"
OPERATOR_STATEMENT_COMMITMENT_DOMAIN = (
    b"kioku/archive-v3/advisory-canary/operator-statement-commitment/v1\x00"
)
IMAGE_ATTESTATION_DOMAIN = b"kioku/archive-v3/advisory-canary/image-attestation/v1\x00"
RUNTIME_ADMISSION_DOMAIN = b"kioku/archive-v3/advisory-canary/runtime-admission/v1\x00"
EMPTY_PHASE1_MUTATION_SET_DOMAIN = (
    b"kioku/archive-v3/advisory-canary/authoritative-mutation-set/empty/v1\x00"
)

FORMAT_V1 = 1
PHASE1_ADVISORY = 1
LEGACY_ACKNOWLEDGEMENT_ONLY = 1
...
def build_attestation(scope_id: bytes, statement_commitment: bytes, image_digest: bytes) -> bytes:
    attestation = FORMAT_V1.to_bytes(2, "big") + scope_id + statement_commitment + image_digest
...
def build_admission(
    scope_id: bytes,
    statement_commitment: bytes,
    image_digest: bytes,
    values: dict[str, bytes],
) -> bytes:
    admission = FORMAT_V1.to_bytes(2, "big")
    admission += scope_id
    admission += statement_commitment
    admission += image_digest
    admission += empty_phase1_mutation_set_commitment()
    ...
    admission += PHASE1_ADVISORY.to_bytes(1, "big")
    admission += (0).to_bytes(2, "big")
    admission += LEGACY_ACKNOWLEDGEMENT_ONLY.to_bytes(1, "big")
...
def main() -> int:
    ...
    operator_signature = sign(Path(args.operator_key), OPERATOR_STATEMENT_DOMAIN + statement)
    ...
    write_evidence(output_dir, "operator-statement.bin", statement)
    write_evidence(output_dir, "operator-signature.bin", operator_signature)
    write_evidence(output_dir, "image-attestation.bin", attestation)
    write_evidence(output_dir, "image-attestation-signature.bin", attestation_signature)
    write_evidence(output_dir, "runtime-admission.bin", admission)
    write_evidence(output_dir, "runtime-admission-signature.bin", admission_signature)
