...
for required in KMS_PROJECT KMS_LOCATION KMS_KEY_RING KMS_KEY GCS_MEDIA_BUCKET \
  ...
  BILLING_ENFORCEMENT_MODE REVIEWER_AUTH_API_KEY REVIEWER_AUTH_UID REVIEWER_AUTH_EMAIL PASSWORD_AUTH_MODE \
...
# This assembler is the image trust boundary, so a hand-authored build secret
# cannot bypass the source-reviewed general-password activation gate.
test "$(value PASSWORD_AUTH_MODE)" = off
test -z "$(value PASSWORD_AUTH_API_KEY)"
test -z "$(value PASSWORD_AUTH_PROJECT_ID)"
test -z "$(value PASSWORD_AUTH_TENANT_ID)"
