...
class EnclaveAdapterTests(unittest.TestCase):
    ...
    def test_state_routes_frozen_predecessors_through_exact_schema_readers(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            config = root / "config.env"
            config.write_text("CONFIG=fixture\n", encoding="utf-8")
        ...
        self.assertFalse(
            verifier.call_args.kwargs["allow_frozen_v0_9_18_schema_12_state"]
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.dict(
                ...
                MODULE,
                "_source_coordinates",
                return_value=("a" * 40, "b" * 40, "v0.9.18", "0.9.18"),
            ), mock.patch.object(
                MODULE, "_config", return_value=config
            ), mock.patch.object(
                MODULE, "_check_config_coordinate"
                ...
                MODULE,
                "_verify_bundle",
                return_value={"metadata": {"schema_version": 12}},
            ) as verifier, mock.patch.object(
                MODULE, "_registry_digest", return_value=digest
            ), mock.patch.object(
                MODULE, "_expected_assets"
    ...
    def test_adapter_bundle_call_requires_current_schema_thirteen_with_exact_digest_uri(self) -> None:
        ...
        self.assertEqual(result["metadata"]["schema_version"], 13)
        self.assertEqual(
    ...
    def test_frozen_v0_9_18_state_accepts_schema_twelve_but_prepare_does_not(self) -> None:
        helper = bundle_fixtures.LocalEvidenceTests()
        image_repository = (
            "us-central1-docker.pkg.dev/kioku-joerodriguez/kioku/kioku-enclave"
        )
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.dict(os.environ, environment, clear=False), mock.patch.object(
                ...
                result = MODULE._verify_bundle(
                    ...
                    bundle_fixtures.DIGEST,
                    image_repository=image_repository,
                    allow_frozen_v0_9_18_schema_12_state=True,
                )
