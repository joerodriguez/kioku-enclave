...
`organizing`, `preparing_recap`, `ready`, or `no_memory`; the body
...
its background retry. A per-item failure is internal automatic work and remains
`processing`; clients are never asked to repair cloud processing. Terminally failed jobs
get a bounded second-chance ladder (one resurrected attempt per hour, capped total
attempts, recent sessions only, never for media-integrity failures), and the summarizer
holds its forward-only cursor over otherwise-empty spans while their failures are inside
