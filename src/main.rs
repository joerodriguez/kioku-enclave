...
async fn main() {
    ...
    let resend_api_key = if test_mode_enabled() {
        std::env::var("RESEND_API_KEY").ok()
    } else {
        Some(
            cp::fetch_secret_from_manager("kioku-resend-api-key", "latest")
                .await
                .unwrap_or_else(|error| panic!("Failed to fetch Resend API key: {error}")),
        )
    };
    let email_from_address = std::env::var("EMAIL_FROM_ADDRESS")
        .unwrap_or_else(|_| "Kioku <notifications@notify.kiokuu.com>".to_string());

    let email_transport: Option<Arc<dyn cp::email_worker::EmailTransport>> = resend_api_key
        .map(|key| cp::email_worker::ResendTransport::new(key, email_from_address))
        .transpose()
        .unwrap_or_else(|error| panic!("Invalid Resend configuration: {error}"))
        .map(|transport| Arc::new(transport) as Arc<dyn cp::email_worker::EmailTransport>);
