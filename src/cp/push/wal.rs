...
const MAX_HANDOFF_BYTES: usize = 43;
const MAX_ERROR_CODE_BYTES: usize = 256;
const MAX_TIMESTAMP_BYTES: usize = 64;
const SCHEMA_TABLE: &str = "archive_v3_wal_push_accepted_schema";
const LEDGER_TABLE: &str = "archive_v3_wal_push_accepted_operations";
const STATE_TABLE: &str = "archive_v3_wal_push_accepted_state";
...
/// Exact local half of one definitive APNs acceptance. Every immutable row
/// binding, mutable predecessor fact, and terminal result fact is fingerprinted
/// before actor admission.
pub(crate) struct PushAcceptedPlan {
...
impl PushAcceptedPlan {
    ...
    fn build(
        ...
        validate_uuid(&delivery_id, true)?;
        validate_uuid(&installation_id, false)?;
        validate_uuid(&collapse_id, true)?;
        validate_handoff(&handoff_handle)?;
        if episode_id <= 0
            || delivery_version <= 0
        ...
        match expected_state {
            ...
            ExpectedState::Retry
                if expected_attempt_count == 0 || expected_error_code.is_none() =>
            {
                return Err(WalIdempotencyError::Malformed);
            }
        ...
        if timestamp_millis(&created_at)? > timestamp_millis(&expected_updated_at)?
            || timestamp_millis(&expected_updated_at)?
                > timestamp_millis(&expected_next_attempt_at)?
            || timestamp_millis(&expected_next_attempt_at)? > timestamp_millis(&accepted_at)?
        {
...
pub(crate) struct PushAcceptedLedger;
...
impl WalLogicalDomainPlan for PushAcceptedPlan {
    type Ledger = PushAcceptedLedger;
    type Output = ();
    ...
    fn canonical_request(&self) -> Result<Zeroizing<Vec<u8>>> {
        ...
        encode_string(&mut request, &self.handoff_handle)?;
        encode_string(&mut request, &self.collapse_id)?;
    ...
    fn apply(&self, transaction: &Transaction<'_>) -> Result<WalReplayResult> {
        let Some(stored) = load_push_row(transaction, &self.delivery_id)? else {
            return Err(WalIdempotencyError::Precondition);
        };
        ...
        let changed = transaction
            .execute(
                "UPDATE push_deliveries
                 SET state='accepted',attempt_count=?1,next_attempt_at=?2,
                     response_status=?3,error_code=NULL,updated_at=?4
                 WHERE delivery_id=?5 AND episode_id=?6 AND installation_id=?7
                   AND delivery_version=?8 AND handoff_handle=?9 AND collapse_id=?10
                   AND state=?11 AND attempt_count=?12 AND next_attempt_at=?13
                   AND response_status IS ?14 AND error_code IS ?15
                   AND created_at=?16 AND updated_at=?17",
                params![
                    next_attempt_count,
                    self.accepted_at,
        ...
        let settled =
            load_push_row(transaction, &self.delivery_id)?.ok_or(WalIdempotencyError::Corrupt)?;
...
impl WalLogicalDomainLedger<PushAcceptedPlan> for PushAcceptedLedger {
    fn lookup(
        connection: &Connection,
        prepared: &PreparedLogicalMutation<PushAcceptedPlan>,
        ...
        let row = connection
            .query_row(
                "SELECT format_version,codec_version,request_fingerprint,
                 ...
                 FROM archive_v3_wal_push_accepted_operations
    ...
    fn resolve_or_apply(
        ...
        transaction
            .execute(
                "INSERT INTO archive_v3_wal_push_accepted_operations
        ...
        let changed = transaction
            .execute(
                "UPDATE archive_v3_wal_push_accepted_state
...
impl StoredPushRow {
    fn from_row(row: &Row<'_>) -> rusqlite::Result<Self> {
        ...
        Ok(Self {
            ...
            handoff_handle: row.get(3)?,
            collapse_id: row.get(4)?,
            state: row.get(5)?,
    ...
    fn matches_identity(&self, plan: &PushAcceptedPlan) -> bool {
        self.episode_id == plan.episode_id
            && self.installation_id == plan.installation_id
            && self.delivery_version == plan.delivery_version
            && self.handoff_handle == plan.handoff_handle
            && self.collapse_id == plan.collapse_id
            && self.created_at == plan.created_at
    }

    fn matches_predecessor(&self, plan: &PushAcceptedPlan) -> bool {
    ...
    fn matches_terminal(&self, plan: &PushAcceptedPlan) -> bool {
        self.matches_identity(plan)
            && self.state == "accepted"
            && self.attempt_count == plan.expected_attempt_count.saturating_add(1)
            && self.next_attempt_at == plan.accepted_at
...
fn load_push_row(connection: &Connection, delivery_id: &str) -> Result<Option<StoredPushRow>> {
    connection
        .query_row(
            "SELECT episode_id,installation_id,delivery_version,handoff_handle,
                    collapse_id,state,attempt_count,next_attempt_at,response_status,
                    error_code,created_at,updated_at
             FROM push_deliveries WHERE delivery_id=?1",
            [delivery_id],
            StoredPushRow::from_row,
        )
        .optional()
        .map_err(|_| WalIdempotencyError::Unavailable)
}

fn require_kind(prepared: &PreparedLogicalMutation<PushAcceptedPlan>) -> Result<()> {
    (prepared.kind_for_owner() == WalOperationKind::PushDelivery)
        .then_some(())
        .ok_or(WalIdempotencyError::ResultUnsupported)
}

fn validate_uuid(value: &str, lowercase: bool) -> Result<()> {
    if value.len() != MAX_UUID_BYTES
        || !value.bytes().enumerate().all(|(index, byte)| match index {
            8 | 13 | 18 | 23 => byte == b'-',
            _ if lowercase => byte.is_ascii_digit() || (b'a'..=b'f').contains(&byte),
...
fn validate_handoff(value: &str) -> Result<()> {
...
fn ensure_schema(transaction: &Transaction<'_>) -> Result<()> {
    match schema_state(transaction)? {
        ...
        LedgerSchemaState::Absent => {
            transaction
                .execute_batch(
                    "CREATE TABLE archive_v3_wal_push_accepted_schema (
                        singleton INTEGER PRIMARY KEY CHECK(singleton=1),
                        format_version INTEGER NOT NULL CHECK(format_version=1),
                        codec_version INTEGER NOT NULL CHECK(codec_version=1)
                     ) STRICT;
                     CREATE TABLE archive_v3_wal_push_accepted_operations (
                     ...
                     CREATE TABLE archive_v3_wal_push_accepted_state (
                        singleton INTEGER PRIMARY KEY CHECK(singleton=1),
                     ...
                     INSERT INTO archive_v3_wal_push_accepted_schema
                        (singleton,format_version,codec_version) VALUES (1,1,1);
                     INSERT INTO archive_v3_wal_push_accepted_state
    ...
    let marker = connection
        .query_row(
            "SELECT format_version,codec_version
             FROM archive_v3_wal_push_accepted_schema WHERE singleton=1",
            [],
    ...
    let state = connection
        .query_row(
            "SELECT row_count,result_bytes
             FROM archive_v3_wal_push_accepted_state WHERE singleton=1",
            [],
            |row| Ok((row.get::<_, i64>(0)?, row.get::<_, i64>(1)?)),
        )
        .optional()
...
mod tests {
    ...
    const DELIVERY: &str = "11111111-1111-4111-8111-111111111111";
    const DELIVERY_TWO: &str = "22222222-2222-4222-8222-222222222222";
    const INSTALLATION: &str = "33333333-3333-4333-8333-333333333333";
    const COLLAPSE: &str = "44444444-4444-4444-8444-444444444444";
    const HANDOFF: &str = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    const HANDOFF_TWO: &str = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    ...
    fn insert_push(connection: &Connection, delivery_id: &str, episode_id: i64, state: &str) {
        let handoff = if delivery_id == DELIVERY {
            HANDOFF
        } else {
            HANDOFF_TWO
        };
        connection
            .execute(
                "INSERT INTO push_deliveries
                 ...
                 VALUES (?1,?2,3,?3,?4,?5,?6,2,?7,503,'provider_retryable',?8,?9)",
                params![
                    episode_id,
                    INSTALLATION,
                    delivery_id,
    ...
    fn plan_for(delivery_id: &str, episode_id: i64) -> PushAcceptedPlan {
        let handoff = if delivery_id == DELIVERY {
            HANDOFF
        } else {
            HANDOFF_TWO
        };
        PushAcceptedPlan::new(
            delivery_id.into(),
            episode_id,
            INSTALLATION.into(),
            3,
    ...
    fn stable_identity_is_kind_scoped_and_request_binds_predecessor_and_acceptance() {
        ...
        let changed = PushAcceptedPlan::new(
            DELIVERY.into(),
            41,
            INSTALLATION.into(),
            3,
    ...
    fn exact_retry_settlement_applies_once_and_replays_without_rewriting() {
        ...
        insert_push(&connection, DELIVERY, 41, "retry");
    ...
    fn pending_predecessor_is_supported_with_its_exact_null_outcome() {
        let mut connection = connection();
        insert_push(&connection, DELIVERY, 41, "pending");
        connection
            .execute(
                "UPDATE push_deliveries SET attempt_count=0,response_status=NULL,
                 error_code=NULL WHERE delivery_id=?1",
                [DELIVERY],
            )
            .unwrap();
        let pending = PushAcceptedPlan::new(
            DELIVERY.into(),
            41,
            INSTALLATION.into(),
            3,
    ...
    fn missing_precondition_rolls_back_lazy_schema_then_same_identity_applies() {
        ...
        assert_eq!(
            connection
                .query_row(
                    "SELECT COUNT(*) FROM sqlite_schema
                     WHERE name LIKE 'archive_v3_wal_push_accepted_%'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    fn every_substituted_row_fact_and_ineligible_state_fails_closed() {
        for mutation in [
            "UPDATE push_deliveries SET episode_id=42",
            "UPDATE push_deliveries SET installation_id='aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'",
            "UPDATE push_deliveries SET delivery_version=4",
            "UPDATE push_deliveries SET handoff_handle='bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb'",
            "UPDATE push_deliveries SET collapse_id='55555555-5555-4555-8555-555555555555'",
            "UPDATE push_deliveries SET state='failed'",
            "UPDATE push_deliveries SET attempt_count=3",
            "UPDATE push_deliveries SET next_attempt_at='2026-08-14T12:00:02.000Z'",
            "UPDATE push_deliveries SET response_status=502",
            "UPDATE push_deliveries SET error_code='different'",
            "UPDATE push_deliveries SET created_at='2026-08-14T11:58:00.000Z'",
            "UPDATE push_deliveries SET updated_at='2026-08-14T12:00:02.000Z'",
        ] {
            let mut connection = connection();
            insert_push(&connection, DELIVERY, 41, "retry");
    ...
    fn exact_preexisting_terminal_is_adopted_but_alternate_acceptance_is_rejected() {
        ...
        insert_push(&adopted, DELIVERY, 41, "retry");
        adopted
            .execute(
                "UPDATE push_deliveries SET state='accepted',attempt_count=3,
                 next_attempt_at=?1,response_status=200,error_code=NULL,updated_at=?1",
                [ACCEPTED_AT],
            )
            .unwrap();
        assert_eq!(
        ...
        insert_push(&conflict, DELIVERY, 41, "retry");
        conflict
            .execute(
                "UPDATE push_deliveries SET state='accepted',attempt_count=3,
                 next_attempt_at=?1,response_status=200,error_code='other',updated_at=?1",
                [ACCEPTED_AT],
            )
            .unwrap();
        assert_eq!(
    ...
    fn capacity_is_reserved_before_mutation_and_existing_replay_survives() {
        ...
        connection
            .execute(
                "UPDATE archive_v3_wal_push_accepted_state SET row_count=?1",
                [i64::from(MAX_ROWS)],
            )
            .unwrap();
        insert_push(&connection, DELIVERY_TWO, 42, "retry");
        assert_eq!(
            execute_prepared_for_owner(
                &mut connection,
        ...
        assert_eq!(
            load_push_row(&connection, DELIVERY_TWO)
                .unwrap()
                .unwrap()
                .state,
            "retry"
    ...
    #[test]
    fn late_ledger_failure_rolls_back_the_push_transition() {
        let mut connection = connection();
        {
        ...
        connection
            .execute_batch(
                "CREATE TRIGGER reject_push_wal_insert
                 BEFORE INSERT ON archive_v3_wal_push_accepted_operations
        ...
        assert_eq!(stored.next_attempt_at, NEXT_AT);
    ...
    fn changed_same_delivery_conflicts_and_result_tamper_fails_closed() {
        ...
        insert_push(&connection, DELIVERY, 41, "retry");
        let fixed = || {
            PushAcceptedPlan::with_operation_id(
                explicit_id(10),
                DELIVERY,
                41,
                INSTALLATION,
        ...
        .unwrap();
        let changed = PushAcceptedPlan::with_operation_id(
            explicit_id(10),
            DELIVERY,
            41,
            INSTALLATION,
        ...
        connection
            .execute(
                "UPDATE archive_v3_wal_push_accepted_operations
    ...
    fn close_reopen_replays_exactly_and_admits_the_next_delivery() {
        ...
        let path = directory.path().join("push.sqlite");
        {
        ...
        insert_push(&reopened, DELIVERY_TWO, 42, "retry");
        assert_eq!(
            execute_prepared_for_owner(
                &mut reopened,
                PreparedLogicalMutation::prepare(plan_for(DELIVERY_TWO, 42)).unwrap(),
