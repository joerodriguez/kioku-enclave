...
| `selected_screenshot_termination.rs` | Inactive definitive-no-object C settlement. It can be constructed only from the non-cloneable rejection proof and caller-fixed canonical observation time. One immediate transaction reauthenticates the unique execution claim, complete B/candidate/`SendStarted`/provider chain, and absence of local/A-v2 settlement before retaining a bounded unit terminal with exact readback. Exact replay/restart never retries the provider. Only the authenticated terminal releases episode count/bytes for another target; the original target remains burned, and tampered/partial state fails closed. It fences A-v2, new candidate admission, and provider request preparation, and has no provider/retry/Store/KMS/list/delete/clock/random/launcher/route/task/acknowledgement authority. |
