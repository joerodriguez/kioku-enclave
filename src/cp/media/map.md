...
Browser-v2 capture is an exact cross-language integrity boundary: state keys bind
the device and lowercase SHA-256 commitment, the commitment covers permission,
active coordinates, counts/truncation, explicit ambient-tab consent, ordered tab
coordinates/title/URL/scheme/activity/loading, and the route enforces UTF-8 byte
bounds plus supported-browser topology. Persisted state uses a versioned envelope;
same-key body drift and key-only missing states fail closed, while browser-v1 replay
remains compatible.
