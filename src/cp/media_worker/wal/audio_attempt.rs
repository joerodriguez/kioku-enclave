//! Inactive pre-provider identity for one leased audio-window attempt
//! (ADR-0022 slice 11, plan A — the structural clone of the sealed screen
//! attempt boundary).
//!
...
use super::audio_result::current_audio_work_attempt_commitments;

const REQUEST_V1: u16 = 1;
const REQUEST_AUDIO_WINDOW_ATTEMPT: u8 = 1;
const RESULT_V1: u16 = 1;
const RESULT_AUDIO_WINDOW_ATTEMPT: u8 = 1;
const EVENT_ID_DOMAIN: &[u8] = b"archive-v3-audio-window-vertex-event-v1\0";
const OPERATION_SOURCE_DOMAIN: &[u8] = b"audio-window-vertex-attempt-v1\0";
const BINDING_DOMAIN: &[u8] = b"archive-v3-audio-window-vertex-binding-v1\0";
...
const SCHEMA_TABLE: &str = "archive_v3_wal_audio_vertex_attempt_schema";
const LEDGER_TABLE: &str = "archive_v3_wal_audio_vertex_attempt_operations";
const STATE_TABLE: &str = "archive_v3_wal_audio_vertex_attempt_state";
...
pub(crate) struct AudioWindowAttemptReceipt {
    event_id: String,
    binding_commitment: [u8; 32],
}

impl AudioWindowAttemptReceipt {
...
/// Stable pre-provider billing and work binding for an audio window.
/// The attempt commitment is minted from the complete currently leased work
/// topology by the private helper in the reviewed result sibling.
pub(crate) struct AudioWindowAttemptPlan {
    ...
    location: String,
    observed_at: String,
    receipt: AudioWindowAttemptReceipt,
}

impl AudioWindowAttemptPlan {
...
pub(crate) struct AudioWindowAttemptLedger;
...
impl WalLogicalDomainPlan for AudioWindowAttemptPlan {
    type Ledger = AudioWindowAttemptLedger;
    type Output = AudioWindowAttemptReceipt;
    ...
    fn canonical_request(&self) -> Result<Zeroizing<Vec<u8>>> {
        ...
        request.push(REQUEST_AUDIO_WINDOW_ATTEMPT);
    ...
    fn apply(&self, transaction: &Transaction<'_>) -> Result<WalReplayResult> {
        let current = current_audio_work_attempt_commitments(
            transaction,
            &self.work_unit_id,
            &self.observed_at,
        )?;
        ...
        transaction
            .execute(
                "INSERT INTO vertex_usage_events
                 ...
                 VALUES (?1,'audio_understanding',?2,NULL,?3,'on_demand',
...
impl WalLogicalDomainLedger<AudioWindowAttemptPlan> for AudioWindowAttemptLedger {
    fn lookup(
        connection: &Connection,
        prepared: &PreparedLogicalMutation<AudioWindowAttemptPlan>,
        ...
        let row = connection
            .query_row(
                "SELECT operation_id,format_version,codec_version,request_fingerprint,
                 ...
                 FROM archive_v3_wal_audio_vertex_attempt_operations
    ...
    fn resolve_or_apply(
        ...
        if u64::from(row_count).saturating_mul(4) >= u64::from(MAX_ROWS).saturating_mul(3) {
            tracing::warn!(
                ledger = LEDGER_TABLE,
                row_count,
                "audio vertex attempt ledger is past three-quarters capacity"
            );
        }
        let plan = prepared.plan_for_domain_ledger();
        ...
        transaction
            .execute(
                "INSERT INTO archive_v3_wal_audio_vertex_attempt_operations
        ...
        let changed = transaction
            .execute(
                "UPDATE archive_v3_wal_audio_vertex_attempt_state
...
impl StoredLedgerRow {
    ...
    fn matches_plan(&self, plan: &AudioWindowAttemptPlan) -> bool {
...
/// Reauthenticates the permanent pre-provider binding for one terminal audio
...
pub(in crate::cp::media_worker) fn authenticate_audio_window_attempt_binding(
    connection: &Connection,
    account_id: &str,
    event_id: &str,
    work_unit_id: &str,
    ...
    let attempt_commitment: [u8; 32] = row
        ...
        .try_into()
        .map_err(|_| WalIdempotencyError::Corrupt)?;
    let expected = AudioWindowAttemptPlan::build(
        None,
    ...
    AudioWindowAttemptLedger::lookup(connection, &prepared)?.ok_or(WalIdempotencyError::Corrupt)?;
...
fn validate_bound_event(connection: &Connection, plan: &AudioWindowAttemptPlan) -> Result<()> {
    let row = connection
        .query_row(
...
fn validate_started_event(connection: &Connection, plan: &AudioWindowAttemptPlan) -> Result<()> {
    let exact = connection
        .query_row(
            "SELECT COUNT(*) FROM vertex_usage_events
             WHERE event_id=?1 AND operation='audio_understanding'
...
fn require_kind(prepared: &PreparedLogicalMutation<AudioWindowAttemptPlan>) -> Result<()> {
...
fn ensure_schema(transaction: &Transaction<'_>) -> Result<()> {
    match schema_state(transaction)? {
        ...
        LedgerSchemaState::Absent => {
            transaction
                .execute_batch(
                    "CREATE TABLE archive_v3_wal_audio_vertex_attempt_schema (
                        singleton INTEGER PRIMARY KEY CHECK(singleton=1),
                        format_version INTEGER NOT NULL CHECK(format_version=1),
                        codec_version INTEGER NOT NULL CHECK(codec_version=1)
                     ) STRICT;
                     CREATE TABLE archive_v3_wal_audio_vertex_attempt_operations (
                     ...
                     CREATE TABLE archive_v3_wal_audio_vertex_attempt_state (
                        singleton INTEGER PRIMARY KEY CHECK(singleton=1),
                     ...
                     INSERT INTO archive_v3_wal_audio_vertex_attempt_schema
                        (singleton,format_version,codec_version) VALUES (1,1,1);
                     INSERT INTO archive_v3_wal_audio_vertex_attempt_state
    ...
    let marker = connection
        .query_row(
            "SELECT format_version,codec_version
             FROM archive_v3_wal_audio_vertex_attempt_schema WHERE singleton=1",
            [],
    ...
    let state = connection
        .query_row(
            "SELECT row_count,result_bytes
             FROM archive_v3_wal_audio_vertex_attempt_state WHERE singleton=1",
            [],
            |row| Ok((row.get::<_, i64>(0)?, row.get::<_, i64>(1)?)),
        )
        .optional()
...
fn encode_receipt(receipt: &AudioWindowAttemptReceipt) -> Result<WalReplayResult> {
    let mut bytes = Vec::with_capacity(105);
    bytes.extend_from_slice(&RESULT_V1.to_be_bytes());
    bytes.push(RESULT_AUDIO_WINDOW_ATTEMPT);
...
fn decode_receipt(result: &WalReplayResult) -> Result<AudioWindowAttemptReceipt> {
    let WalReplayResult::CanonicalResponse(bytes) = result else {
        return Err(WalIdempotencyError::ResultUnsupported);
    };
    let mut reader = Reader::new(bytes);
    if reader.take_u16()? != RESULT_V1 || reader.take_u8()? != RESULT_AUDIO_WINDOW_ATTEMPT {
        return Err(WalIdempotencyError::Corrupt);
    }
...
mod tests {
    ...
    use crate::cp::media_worker::wal::audio_result::tests::{
        attempted_at, install_schema, reserved_usage, seed_reserved_audio_work, ACCOUNT,
        LEASE_UNTIL, MODEL, WORK_ONE,
    };
    ...
    fn plan_at(
        ...
        location: &str,
        observed_at: &str,
    ) -> AudioWindowAttemptPlan {
        let commitments =
            current_audio_work_attempt_commitments(connection, work, observed_at).unwrap();
        AudioWindowAttemptPlan::new(
            ACCOUNT.to_owned(),
            work.to_owned(),
    ...
    fn execute(
        ...
        plan: AudioWindowAttemptPlan,
    ) -> std::result::Result<
        crate::archive_v3_wal_idempotency::ExecutedLogicalMutation<AudioWindowAttemptPlan>,
        WalIdempotencyError,
    > {
    ...
    // T1 — no aliasing against the sealed screen family for the same
    // durable inputs (the domain strings themselves are additionally
    // registered pairwise-distinct in archive_v3_wal_idempotency's alias
    // test).
    #[test]
    fn t01_audio_identity_never_aliases_the_screen_family() {
        let connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        let observed = attempted_at(1);
        let commitments =
            current_audio_work_attempt_commitments(&connection, WORK_ONE, &observed).unwrap();
        let audio = AudioWindowAttemptPlan::new(
            ACCOUNT.to_owned(),
            WORK_ONE.to_owned(),
            commitments.predecessor(),
            commitments.attempt(),
        ...
        .unwrap();
        let screen = super::super::attempt::ScreenStoryboardAttemptPlan::new(
            ACCOUNT.to_owned(),
            WORK_ONE.to_owned(),
            commitments.predecessor(),
            commitments.attempt(),
        ...
        .unwrap();
        assert_ne!(audio.operation_id(), screen.operation_id());
        // The derived event id also differs from any bare-source id of the
        // same kind.
        assert_ne!(
            audio.operation_id(),
            WalLogicalOperationId::from_stable_source(
                WalOperationKind::VertexUsage,
                audio.receipt.event_id.as_bytes(),
            )
            .unwrap()
        );
    }

    // T2 — clock freedom: the identity is a pure function of the durable
    // topology; two wall clocks derive the identical event id, and the
    // ledger replays the byte-equal receipt for the identical plan.
    #[test]
    fn t02_same_topology_derivation_is_clock_free_and_replays_exactly() {
        let mut connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        let first_clock = plan_at(
            &connection,
            WORK_ONE,
            MODEL,
            "us-central1",
        ...
        let second_clock = plan_at(
            &connection,
            WORK_ONE,
            MODEL,
            "us-central1",
            "2026-08-15T15:00:04.000Z",
        );
        assert_eq!(first_clock.receipt.event_id, second_clock.receipt.event_id);
        assert_eq!(first_clock.operation_id(), second_clock.operation_id());
        ...
        let receipt = execute(&mut connection, first_clock)
            .unwrap()
    ...
    // T3 — a genuine re-claim advances the fingerprinted topology, so the
    // event id and operation id both change (B).
    #[test]
    fn t03_reclaim_derives_a_new_identity() {
        let mut connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        let first = plan_at(
            &connection,
        ...
        assert_ne!(retry.operation_id(), first_operation);
        assert_eq!(
    ...
    // T4 — a screen-class unit can never mint an audio attempt.
    #[test]
    fn t04_screen_class_unit_is_refused() {
        let connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        connection
            ...
            .unwrap();
        assert_eq!(
            current_audio_work_attempt_commitments(&connection, WORK_ONE, &attempted_at(1))
                .unwrap_err(),
            WalIdempotencyError::Precondition
        );
    }

    // T5 — the reservation is pinned to the audio class constant (4096);
    // the legacy in-function test shape (reserved 1024) must never pass.
    #[test]
    fn t05_reserved_tokens_are_pinned_to_the_audio_class_constant() {
        let connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        let downgraded = serde_json::json!({
            "work_unit_id": WORK_ONE,
            "work_class": "audio",
            "member_count": 1,
        ...
        connection
            .execute(
                "UPDATE media_work_units SET reserved_output_tokens=1024,usage_json=?1",
                [&downgraded],
            )
            .unwrap();
        connection
    ...
    // T6 — predecessor drift, an early attempt time, and a sub-two-minute
    // lease window all refuse before any write.
    #[test]
    fn t06_exact_stage_and_attempt_window_are_required_before_any_write() {
        for mutation in ["job", "early", "short", "expired"] {
            let mut connection = connection();
            seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
            let commitments =
                current_audio_work_attempt_commitments(&connection, WORK_ONE, &attempted_at(1))
            ...
            let observed_at = match mutation {
                "early" => "2026-08-15T14:59:58.000Z".to_owned(),
                "short" => "2026-08-15T15:03:01.000Z".to_owned(),
                "expired" => "2026-08-15T15:05:01.000Z".to_owned(),
                _ => attempted_at(1),
            };
            if mutation == "job" {
                connection
                    .execute(
                        "UPDATE media_processing_jobs SET updated_at='2026-08-15T15:00:02.500Z'",
                        [],
                    )
                    .unwrap();
            }
            let plan = AudioWindowAttemptPlan::new(
                ACCOUNT.to_owned(),
                WORK_ONE.to_owned(),
                commitments.predecessor(),
                commitments.attempt(),
    ...
    // T7 — an existing (unledgered) event row is never adopted.
    #[test]
    fn t07_existing_event_id_is_never_adopted() {
        let mut connection = connection();
        seed_reserved_audio_work(&connection, WORK_ONE, 1, 1);
        let plan = plan_at(
            &connection,
        ...
        connection
            .execute(
                "INSERT INTO vertex_usage_events
                 ...
                 VALUES (?1,'audio_understanding',?2,NULL,'us-central1','on_demand',
                         NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,
                         'started','pending',0,?3,?3)",
                params![plan.receipt.event_id, MODEL, attempted_at(1)],
            )
            .unwrap();
        assert_eq!(
            execute_error(&mut connection, plan),
    ...
    // T8 — the acoustic route facts are inside the attempt topology: any
    // change shifts the commitment (and with it the derived identity).
    #[test]
    fn t08_audio_route_facts_shift_the_attempt_commitment() {
        let baseline_connection = connection();
        seed_reserved_audio_work(&baseline_connection, WORK_ONE, 1, 1);
        let baseline = current_audio_work_attempt_commitments(
            &baseline_connection,
            WORK_ONE,
            &attempted_at(1),
        )
        .unwrap();
        for mutation in [
            "UPDATE capture_events SET audio_role='local_transmit'",
            "UPDATE capture_events SET audio_route='bluetooth'",
            "UPDATE capture_events SET route_epoch=2",
        ] {
            let shifted = connection();
            seed_reserved_audio_work(&shifted, WORK_ONE, 1, 1);
            shifted.execute(mutation, []).unwrap();
            let moved =
                current_audio_work_attempt_commitments(&shifted, WORK_ONE, &attempted_at(1))
                    .unwrap();
            assert_ne!(moved.attempt(), baseline.attempt(), "{mutation}");
            assert_ne!(moved.predecessor(), baseline.predecessor(), "{mutation}");
        }
    }

    // Structural clone: the full-column started row, coverage advance, and
    // typed 114-byte receipt survive close/reopen replay.
    #[test]
    fn exact_attempt_applies_once_and_survives_close_reopen() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("audio-attempt.sqlite");
        let (replay, event_id) = {
            ...
            assert_eq!(
                connection
                    .query_row(
                        ...
                        params![event, MODEL, attempted_at(1)],
                        |row| row.get::<_, i64>(0),
                    )
                    .unwrap(),
                1
        ...
        assert_eq!(load_ledger_state(&reopened).unwrap(), (1, 114));
    }

    #[test]
    fn constructors_reject_unstable_or_malformed_boundary_inputs() {
        assert!(AudioWindowAttemptPlan::new(
            ACCOUNT.to_owned(),
            WORK_ONE.to_owned(),
            [0; 32],
            [1; 32],
        ...
        .is_err());
        assert!(AudioWindowAttemptPlan::with_operation_id(
