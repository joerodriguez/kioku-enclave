...
| `control_store.rs` | Identity + accounting in the context-bound encrypted SQLite blob `control/control.db.enc` (users, deletion tombstones, OAuth clients/codes, refresh tokens, usage, redacted query log, and encrypted webhook destinations/signing secrets); migration images rewrite legacy encryption on load |
