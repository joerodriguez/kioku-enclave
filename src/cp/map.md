...
| `summarizer.rs` | v2 incremental episode summarizer + internal tokio cron (replaces Cloud Scheduler). Open-episode context carries type, participants, summary, and timeline so the model extends a shared real-world workflow across transfers, reconnects, apps, and sequential calls instead of fragmenting it. Single-pass eager generation: title, exec summary, minute-timeline gists, and ADR-0009 substance; guarded one-time batched historical substance backfill stored in per-user metadata. After upsert it embeds each episode in-enclave (candle, pinned MODEL_ID → `vec_episodes`, §G.2). Live-tail cursor semantics hold short/empty tails as documented in the module |
