...
| `reconciler.rs` | Prompt bounded memory organization with eight-hour capture-time continuation context; exhaustive validated partitions, staged results, providerless oversized scans, atomic identity/lineage publication and durable activation authority. |
...
| `vertex.rs` | Bounded Vertex Gemini adapter with strict schemas and frozen media-result contract versions, content-free usage metadata, and explicit model routing for separately qualified reconciliation. |
...
| `voice_quality.rs` | Versioned enrollment/matching quality policy (quality 1 with the level-relative speech-frame detector 2: floor -50 dBFS, within ~16 dB of the chunk's loud frames, absolute rule kept for signals with under 12 dB of dynamic range) and gate-driven best-span selection inside long turns. |
