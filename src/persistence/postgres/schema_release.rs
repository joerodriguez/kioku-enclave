...
/// The compiled reconciliation producer contract this package must emit. The
/// release pipeline independently recomputes it and refuses a supplied label
/// mismatch; pinning it here fails a source change closed at test time too.
///
...
const EXPECTED_RECONCILIATION_PRODUCER_CONTRACT: &str =
    "sha256:b4fc5bf74543c944f86a2929bc5d33b855a4ab17bf079e7dd4642bc0352ecb6b";
...
mod tests {
    ...
    /// The compiled producer contract is release-binding: the image pipeline
    ...
    fn compiled_producer_contract_matches_the_release_pin() {
        let producer =
            crate::cp::reconciler::producer_contract_commitment("gemini-3.5-flash", "global")
                .unwrap();
        assert_eq!(
            sha256_label(&producer),
            EXPECTED_RECONCILIATION_PRODUCER_CONTRACT
        );
    }

    /// The retired v0.9.31 bridge and its redrain twin must not come back: a
    /// package-version-scoped escape from the exact activation authority is
