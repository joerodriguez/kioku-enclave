...
pub(super) fn episode_from_row(row: &sqlx::postgres::PgRow) -> Result<FinalizedEpisode> {
    Ok(FinalizedEpisode {
...
impl DeliveryRepository for PostgresPersistence {
    ...
    async fn next_email_candidate(
        ...
        super::morning_email::next_candidate(self, account_id).await
    }
    ...
    async fn claim_email(
        ...
        super::morning_email::claim(self, candidate, request, lease_seconds).await
    }
    ...
    async fn settle_email(
        ...
        if claim.delivery_id.starts_with("digest_") {
            return super::morning_email::settle(self, claim, outcome, circuit_seconds).await;
        }
        if !outcome.is_valid()
