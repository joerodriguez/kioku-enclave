...
| `aggregate_audit.rs` / `aggregate_audit.sql` / `aggregate_audit_fixture.json` | Fixed, content-free v3 post-deploy PostgreSQL aggregate snapshot in a repeatable-read, read-only transaction. Formation stream diagnostics use the runtime accepted/contiguous functions to distinguish real sequence gaps from live-export gaps bridged by deletion tombstones; they never replace readiness gates. Model-provider quiescence counts raw intents and worker authority across all dates/accounts, including expired or residual ownership, independently of cumulative quota debits. Its staged readiness keeps every shared safety gate on both transitions, excludes phase-gated formation repair only from `ready_for_drain`, and requires formation quiescence for `ready_for_active`. Capacity reserves one reconciliation call per candidate draft and sums all canonical-owned atoms plus individual source-less draft floors, then adds each candidate-bearing account's unowned atoms once. This survives deletion splitting components; serializable publication, source revalidation, and unique active ownership prevent reuse. Daily quotas, the 65-slot reserve, oversized-component refusal, and existing-finalizer costs are unchanged. |
