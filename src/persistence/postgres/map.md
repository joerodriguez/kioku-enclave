...
| `aggregate_audit.rs` / `aggregate_audit.sql` / `aggregate_audit_fixture.json` | Fixed, content-free v2 post-deploy PostgreSQL aggregate snapshot in a repeatable-read, read-only transaction. Model-provider quiescence counts raw intents and worker authority across all dates/accounts, including expired or residual ownership, independently of cumulative quota debits. Its staged readiness keeps every shared safety gate on both transitions, excludes phase-gated formation repair only from `ready_for_drain`, and requires formation quiescence for `ready_for_active`. Capacity matches runtime draft eligibility and conservatively bounds each component's successor finalizers by its canonical owned atoms plus all account-unowned atoms, capped at 32 with a draft-count floor for providerless KEEP; daily quotas, the 65-slot reserve, and existing-finalizer costs are unchanged. |
