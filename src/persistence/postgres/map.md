...
| `activation.rs` | Append-only v27 install/backfill/drain/activate/pause/resume authority, shared/exclusive release-lock serialization before schema probes, sticky scope, durable candidate-fleet image identity, exact catalog/receipt verification, database guard installation, claim drain, and runtime/repository gates. Installed backfill also clears bounded expired terminal media ownership only for accounts without unfinished media, live deadlines, or started provider intents; lifecycle fencing preserves intent admission and all history/quota/content remains unchanged. |
