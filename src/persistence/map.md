...
| `capture.rs` | Atomic capture/reference preflight, commit, replay, deletion-tombstone no-resurrection, session/event status, and bounded interrupted-session recovery contract; canonical upload reservations carry session/stream/event/asset identity for atomic erasure admission. |
...
| `memory_reconciliation.rs` | Current-formation cohort snapshots, eight-hour capture context, fleet leases, the per-attempt frozen provider request (exact rendered input plus its authoring namespace, bound to the claim's attempt identity), staged partitions, bounded discovery/verification, atomic active-topology publication and durable handle resolution. |
| `model_usage.rs` | Vertex intent/outcome (a re-entered identity with different bytes is refused after the owner-lost rule settles a started row, and its stored provenance stays readable), billing batch claim, and coverage reconciliation contract. |
...
| `playback.rs` | Memory and exact capture-session playback datasets plus identified/recurring-person memory availability. |
| `query.rs` | Tenant-scoped full-text/vector search over memories, structured final briefs, transcripts, and screen evidence; stable memory navigation and identified/recurring-person link projections and separately paged People summaries; MCP query projections; turn-timed episode members; and feed/people/browser/screenshot reads plus capture status. |
| `recording_retention.rs` | Retention preview/CAS, durable recording-key epoch, exact inventory, and downgrade completion. |
| `voice_identity.rs` | Versioned source/claim/sample outcomes, operator cohort snapshots, closed enrollment states/reasons, current domain recognition, atomic Forget, source expiry and bounded provider-free stored-sample maintenance contracts. |
