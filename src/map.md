...
| `store.rs` | Per-user encrypted SQLite blob storage in GCS (load → decrypt → mutate → encrypt → persist); schema/migrations include ADR-0009 episode substance and per-user backfill markers |
