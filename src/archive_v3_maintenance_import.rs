...
impl SingleArchiveMaintenanceImporter {
    ...
    pub(crate) async fn run(
        ...
        let CompletedMaintenanceImport::WalAuthoritative(handoff) = completed;
        Ok(*handoff)
    }
...
pub(crate) mod tests {
    ...
    fn maintenance_surface_is_inactive_redacted_and_has_no_delete_or_list() {
        ...
        // The AdvisoryShadow import target was DELETED with the Phase-2 /
        // advisory-owner family (genesis-first replan). Pinned absent, so a
        // revival arrives as a reviewed design rather than a stray variant.
        assert!(!source.contains(concat!("MaintenanceImportTarget::Advisory", "Shadow")));
        // WalAuthoritative may be selected by exactly the two known run_owned
        // spawn expressions — run() and run_phase2(). A third selection site
        // requires deliberately updating this count.
        assert_eq!(
            source
                .matches(concat!(
        ...
        // The advisory (ShadowWal) release predicate's call sites here went
        // with the deleted advisory import arm; the WalAuthoritative terminal
        // variant is the one reconstruction still authenticates against.
        ...
        // The exact-terminal double-read (load, validate, re-load,
        // re-validate). Two sites: both in finish_offline_import. The other
        // pair lived in the deleted advisory finish arm.
        assert_eq!(
            source
                .matches(concat!(
